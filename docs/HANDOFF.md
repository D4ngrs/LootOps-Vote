...
- Cloudflare Worker deployed at `https://lootops-vote-worker.lootopsd4.workers.dev`. Secrets (`DISCORD_BOT_TOKEN`, `DISCORD_CLIENT_SECRET`, `SESSION_SIGNING_SECRET`) are set via `wrangler secret put` — **not** in any file. The old `SHARED_AUTH_SECRET` shared-secret system has been retired entirely (see `docs/superpowers/specs/2026-08-03-officer-oauth-auth-design.md` and `docs/superpowers/plans/2026-08-03-officer-oauth-auth.md`): access is now real Discord OAuth, gated on the `OFFICER_ROLE_IDS` var (comma-separated, `worker/wrangler.toml`) checked live against the officer's Discord roles at login. To run manual test scripts against protected endpoints, log in via a browser first and copy the resulting session token out of `localStorage`'s `rollcall_vote_session_v1` key; don't ask the user for a static credential.
- `LootOps-Vote` settings (Worker URL, history webhook URL) and the session token (`rollcall_vote_session_v1`) are stored in the browser's `localStorage` per-origin — they do **not** carry over between `d4ngrs.github.io/LootOps-Vote/` (the real site) and any `localhost` test server. Re-enter/re-login each time you spin up a local server for testing. Note `ALLOWED_RETURN_ORIGINS` in `worker/wrangler.toml` must include whatever origin you're testing from, or `/auth/login` will reject the redirect — currently only `https://d4ngrs.github.io` is allowed, so local-server logins won't work until you add e.g. `http://localhost:PORT` to that comma-separated list and redeploy.
- Login: the app's own front-page password gate has been removed entirely — the full-page login gate (`#loginGate` in `index.html`) is now the Discord OAuth login itself, and is the sole gate for both viewing the app and calling the Worker (one session token, checked by both). There is no bypass; testing requires logging in with a real Discord account that holds one of the roles in `OFFICER_ROLE_IDS`.
