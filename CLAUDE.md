...
The copy is only a starting point, not a constraint — this repo is free to diverge from plain LootOps wherever the voting feature needs it to (e.g. manual name entry is planned to be removed, per the design spec). That said, by default this repo still follows the same conventions documented below (the Names/Items uniform row-height requirement, changelog/version-bump rules, etc.) — those aren't automatically waived by the fork. Any departure from them is a deliberate, specifically-discussed decision, not an assumed default.

**The single-file, no-build-step structure is not a requirement or preference of the person running this fork.** It's simply the current state of the code, inherited unchanged from the original LootOps repo. Do not cite "single-file convention" or "no build step" as a reason to avoid restructuring, and do not describe staying single-file as something the user wants — they've corrected this assumption multiple times. A move to a plain ES-module split (no framework, no bundler) has been actively discussed and is a live, open possibility — see `docs/HANDOFF.md`.
